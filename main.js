...
client.on('message', msg => { 
    let spl = msg.content.substring(prefix.length).split('\n');
    var spl_size = spl.length;
    var splrr;

    for(splrr=0;splrr<spl_size;splrr++){
      let arg = spl[splrr].content.substring(prefix.length).split(' ');
      if(!spl[splrr].content.startsWith(prefix))return;
      if(arg[2]=='error'){
